...
object DisplayLauncher {
    private const val TAG = "ThorUIDisplay"
    const val EXPECTED_DISPLAY_ID = "expected_display_id"

    fun launchCompanion(activity: Activity): Boolean {
        ...
        val target = DisplayPolicy.chooseCompanion(currentId, displayProfiles(activity, intent))
            ?: return false
        intent.putExtra(EXPECTED_DISPLAY_ID, target.id)
    ...
    private fun displayProfiles(activity: Activity, intent: Intent): List<DisplayProfile> {
        ...
        return manager.displays.map { display ->
            DisplayProfile(
                ...
                launchAllowed = isLaunchAllowed(activity, activityManager, display, intent),
            )
        }
    }
    ...
    private fun isLaunchAllowed(
        ...
        display: Display,
        intent: Intent,
    ): Boolean = Build.VERSION.SDK_INT < Build.VERSION_CODES.Q ||
        manager.isActivityStartAllowedOnDisplay(activity, display.displayId, intent)
